...
Адаптируем код с сайта Embassy для STM32F411CE. Создадим новый проект с названием `start`. В `Cargo.toml` необходимо добавить поддержку EXTI:
...
embassy-stm32 = { version = "0.6.0", features = [
  ...
  "memory-x",
  "time-driver-any",
  "exti", # добавить эту строку
] }
...
```
В `src/main` перепишем адаптированный код:
